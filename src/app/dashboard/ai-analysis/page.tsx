...
import { runAnalyticsAgent, getAgentHistory, clearAgentHistory } from "@/actions/analytics-agent-actions";
...
const AGENT_SUGGESTIONS = [
  "What are my top 5 best-selling categories?",
  "Which products are critically low on stock?",
  "Show me revenue trends for the last 7 days",
  "Who are my highest-value customers?",
  "What's the average order value and how is it trending?",
  "Are there any pricing anomalies in Electronics?",
];
...
export default function AIAnalysisPage() {
  ...
  // Generate a stable session ID for the agent chat
  const [sessionId] = useState(() => `analytics-${Date.now()}`);

  const handleAgentSend = useCallback(
    async (message: string, sid: string) => {
      const result = await runAnalyticsAgent(message, sid);
      return result;
    },
    []
  );
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm md:text-base">
            Interactive AI agents and automated RAG analysis for your store data.
          </p>
        </div>
      ...
      {activeTab === "agent" && (
        ...
        >
          <Card className="border-t-4 border-t-indigo-500 bg-white/40 dark:bg-black/40 backdrop-blur-md shadow-xl overflow-hidden">
            <div className="h-[600px]">
              <AgentChat
                ...
                agentName="Analytics ReAct Agent"
                agentColor="indigo"
          ...
          <div className="grid gap-4 md:grid-cols-3 mt-6">
            <Card className="bg-white/40 dark:bg-black/40 backdrop-blur-md border border-indigo-500/10">
              <CardContent className="p-4 flex items-start gap-3">
                <div className="p-2 rounded-lg bg-indigo-500/10">
                  <Brain className="h-5 w-5 text-indigo-500" />
                </div>
                <div>
                  <h4 className="font-semibold text-sm">Multi-Step Reasoning</h4>
                  <p className="text-[11px] text-muted-foreground mt-1">
                    Uses ReAct (Reason + Act) loops to chain database queries and build comprehensive answers.
                  </p>
                </div>
              </CardContent>
            </Card>
            <Card className="bg-white/40 dark:bg-black/40 backdrop-blur-md border border-emerald-500/10">
              <CardContent className="p-4 flex items-start gap-3">
                <div className="p-2 rounded-lg bg-emerald-500/10">
                  <Database className="h-5 w-5 text-emerald-500" />
                </div>
                <div>
                  <h4 className="font-semibold text-sm">Live Data Access</h4>
                  <p className="text-[11px] text-muted-foreground mt-1">
                    10 real-time tools: query products, orders, revenue metrics, customer segments, and more.
                  </p>
                </div>
              </CardContent>
            </Card>
            <Card className="bg-white/40 dark:bg-black/40 backdrop-blur-md border border-amber-500/10">
              <CardContent className="p-4 flex items-start gap-3">
                <div className="p-2 rounded-lg bg-amber-500/10">
                  <MessageSquare className="h-5 w-5 text-amber-500" />
                </div>
                <div>
                  <h4 className="font-semibold text-sm">Conversation Memory</h4>
                  <p className="text-[11px] text-muted-foreground mt-1">
                    Context-aware chat with persistent memory. Follow up on previous questions naturally.
                  </p>
                </div>
              </CardContent>
            </Card>
      ...
      {/* ─── TAB: RAG Analyst (existing UI preserved) ─── */}
      {activeTab === "rag" && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
